...
const ForEvent = ({ curElem }) => {
    ...
    const handleOnSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/v1/booking/for-Event', formData, {
                headers: {
            ...
            if (response.status === 200) {
                ...
                //navigate(`/pay/${response.data.data.bookingForEvent._id}`);
                navigate('/pay-ForEvent', { state: { id: response.data.data.bookingForEvent._id} });
            }
        } catch (error) {
            // Handle error
     ...
     <form className="ease-in duration-300 w-[80%] sm:w-max shadow-sm backdrop-blur-md bg-white/80 lg:w-max mx-auto rounded-md px-8 py-5 mb-8" onSubmit={handleOnSubmit}>
       ...
       <NavLink to="/">
         <img src={logo} alt="" className="logo mx-auto mb-4 h-[6rem] w-[6rem] cursor-pointer text-center"/>
            ...
            <h2 className="text-center mb-6 text-3xl border-solid border-b-2 border-[#c1dadf] font-semibold"> Cook for an Event</h2>
            ...
            <div className='flex items-center'>
                ...
                <p className='px-2 text-[0.75rem]'> If number of people exceeds than 15, then extra ₹150 is applicable per person. </p>
            </div>
