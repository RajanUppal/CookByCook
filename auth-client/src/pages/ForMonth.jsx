...
const ForMonth = ({ curElem }) => {
    ...
    const handleOnSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/v1/booking/for-month', formData, {
                headers: {
            ...
            if (response.status === 200) {
                ...
                //navigate(`/pay/${response.data.data.bookingForMonth._id}`);
                navigate('/pay-ForMonth', { state: { id: response.data.data.bookingForMonth._id} });
            }
        } catch (error) {
            // Handle error
     ...
     <form className="ease-in duration-300 w-[80%] sm:w-max shadow-sm backdrop-blur-md bg-white/80 lg:w-max mx-auto rounded-md px-8 py-5 mb-8" onSubmit={handleOnSubmit}>
       ...
       <NavLink to="/">
         <img src={logo} alt="" className="logo mx-auto mb-4 h-[6rem] w-[6rem] cursor-pointer text-center"/>
            ...
            <h2 className="text-center mb-6 text-3xl border-solid border-b-2 border-[#c1dadf] font-semibold"> Cook for a Month</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 items-center gap-4">
                    ...
                    <label className='text-sm text-gray-400' htmlFor=""> Booking Start Date
                    <input type="date" placeholder="Booking Start Date" name='date' className='shadow-sm bg-white appearance-none border rounded w-full py-3 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' onChange={handleInputChange}/>
                    </label>

                    <label className='text-sm text-gray-400' htmlFor=""> Visit Timing
            ...
            <div className='flex items-center'>
                ...
                <p className='px-2 text-[0.75rem]'> If number of people exceeds than 2, then extra ₹500 is applicable per person. </p>
            </div>
